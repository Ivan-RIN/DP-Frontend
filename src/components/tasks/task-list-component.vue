			...
			<template v-for="i in taskListGroup">
				<div v-for="brigade in i.brigades"
					...
					<div v-for="category in brigade.values"
						...
						<div data-values style="min-height: 70px;">
							<div
								class="task-list-content-brigade-category-task"
								v-for="(task, index) in category.values"
								:id="`task${task.id}`">
	...
	methods: {
		...
		filterList() {
			...
			for (let i = 0; i < array.length; i++) {
				...
				if ((arrayItem.facility.name.toString()
						.toLowerCase()
						.includes(cdngLower)
					|| (arrayItem.facility.field != null
						&& arrayItem.facility.field.name.toString()
							.toLowerCase()
							.includes(cdngLower)))
					&& arrayItem.author.name.toString()
						.toLowerCase()
						.includes(authorLower)
					&& arrayItem.executor.name.toString()
						.toLowerCase()
						.includes(executorLower)) {
					this.newInputCollection.push(arrayItem);
				} else if (arrayItem.facility.parent != null) {
					if (arrayItem.facility.parent.name.toString()
							.toLowerCase()
							.includes(cdngLower)
						&& arrayItem.author.name.toString()
							.toLowerCase()
						...
						&& arrayItem.facility.parent.parent.name.toString()
							.toLowerCase()
							.includes(cdngLower))
						&& arrayItem.author.name.toString()
							.toLowerCase()
		...
		openAddExecutor(task) {
			if (this.$can('read', 'Tasks')) {
				...
				this.$modal.show(TaskExecutor, { task }, {
					height: 'auto',
					width: '400px'
				}, event);
		...
		selectAll() {
			this.taskListGroup.forEach((i) => {
				i.brigades.forEach((brigade) => {
					this.brigadeSelect(brigade, this.isCheckAll);
				});
			});
		},
		...
		showCategoryTasks(category, show) {
			if (show == undefined) category.show = !category.show;
			else category.show = show;
			if (category.show) category.values.push(...category.grouped);
			else category.values = category.values.splice(0, 1);
		},
