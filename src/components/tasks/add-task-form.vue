...
<!--          </div>-->
          <div class="add-task-content-data-field" data-field>
            ...
            <textarea v-model="selectedFacilities" style="height: 30px; width: 530px; resize: none; color: white" disabled="true"></textarea>
            <div @click="clearFacilities()" style="height: 30px; background-color: #2296f3; border-radius: 2px; line-height: 28px; padding: 0 10px; margin: 0 0 0 10px; cursor: pointer">Очистить</div>
          </div>
        </div>
        ...
        if (!this.isFacilityDisabled()) {
          ...
          if (this.selectedWell == '0') {
            for (let i = 0; i < this.wells.length; i++) {
              ...
              this.selectedFacilities.push(' ' + well.name.trim().toString());
      ...
      // },
		...
		methods:
                ...
                if (me.selectedCluster == undefined) {
                  ...
                  me.task.facilityIds.push(parseInt(me.selectedCluster, 10));
					...
					setMinDateForTaskCreate() {
						...
						this.minDate = yesterday.toISOString().substring(0, 10);
					},
          ...
          chooseField() {
            ...
            const clusters_fields = clusters.map(function(item) { if (item.field !== null) return item.field.name; });
            ...
            uniq_fields.forEach((element) => {
              ...
              fields.push({ id: id.field.id, name: element });
            });
            return fields;
            // }
          },
          ...
          chooseWell() {
            ...
            if (wells.length != 0) {
              const all = { id: 0, name: 'Выбрать все скважины' };
          ...
          showFacilitiesSearchModal() {
					...
					returnFileId(data) {
						...
						return this.sendFile(data).then(data => data);
					},
