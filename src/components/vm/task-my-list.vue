<template>
    <div class="task-list">
        <div style="display: flex;">
            <select disabled>
                <option>
                    {{ getNameDepartment(currentUser.departmentId) }}
                </option>
            </select>
            <div style="width: 100%;">
            </div>
        ...
        <div class="">
            ...
            <div v-if="initiatorTasks.length" class="vm-task-list-content-background" style="margin-bottom: 20px;">
                <div class="vm-task-list-content"
                     v-for="task in initiatorTasks"
                     :key="task.id"
            ...
            <div v-else class="vm-task-list-content-background" style="margin-bottom: 20px; padding: 10px 20px;">
                Список задач пуст.
            </div>
            <div style="padding: 8px 8px 8px 8px; font-weight: bold; background-color: #0d304b; min-width: 170px; text-align: center;
            position: relative; top: 0px; display: inline-block; border-radius: 10px 10px 0 0; text-decoration: underline;">
            ...
            <div v-if="executorTasks.length" class="vm-task-list-content-background" style="margin-bottom: 20px;">
                <div class="vm-task-list-content"
                     v-for="task in executorTasks"
                     :key="task.id"
                     @click="openViewTask(task)">
                    <div class="row-col-1" :style="{ color: overdueDateColor(task.state, task.executionDate) }">
                        {{ convertDate(task.executionDate) }}
    ...
    methods: {
        ...
        getMyTasks() {
            for(let i in this.tasks) {
                for(let task of this.tasks[i]) {
                    if (task.initiator.id == this.currentUser.id) this.initiatorTasks.push(task);
                    if (task.executor.id == this.currentUser.id) this.executorTasks.push(task);
                }
            }
        },
    },
