	...
	<div>
		<dp-modal>
			...
			<template slot="body">
				...
				<div class="add-task-content-data" v-if="currentUser.access.isAdministrator">
					<div class="add-task-content-data-field" data-field>
						<div data-name>Инициатор:<span class="requiredField">*</span></div>
						<div data-value>
							<select-users
								v-if="initiator"
								:current="initiator"
...
import SelectUsers from '@/components/vm/select-users';
...
export default {
	...
	methods: {
		...
		init() {
			this.initiator = this.currentUser;
			this.task.boardId = this.board.id;
			this.task.initiatorId = this.currentUser.id;
			if (this._task) {
				this.task = this._task;
				this.endDate = this._task.endDate;
				this.initiator = this.users[this._task.initiatorId];
		...
		validateTaskFields() {
			...
			if (this.initiator) {
				this.task.initiatorId = this.initiator.id;
			}
