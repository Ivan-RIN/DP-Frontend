...
export default {
	...
	methods: {
		...mapActions('storage', ['appendTasks']),
		...
		getTasksForCalendar() {
			...
			const filter = {
				...
				dateFrom: new Date(year, month - 1, 2).toISOString()
					.substring(0, 10),
		...
		async actionGroupApply(action) {
			const taskId = this.taskList.filter(t => t.isCheck)
				.map(t => t.id);
		...
		actionFilterByDateApply(action) {
			if (action === '1') {
				this.filtered.group = this.filtered.group.sort((a, b) => {
					const first = a.id.toString()
						.split('.')
						.reverse()
						.join('.');
					const second = b.id.toString()
						.split('.')
						.reverse()
						.join('.');
